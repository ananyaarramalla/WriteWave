...
function Article() {
  ...
  const writeComment = async (commentObj) => {
    ...
    console.log("Sending articleId:", state.articleId); // Log the articleId
    try {
      ...
      if (res.data.message === "Comment posted") {
        ...
        // Optionally, update local state to show the new comment
      ...
      {articleEditStatus === false ? (
        <>
          <div className="d-flex justify-content-between">
            <div>
              <p className="display-3 me-4 text-white title">{state.title}</p>
              <span className="py-3">
                <small className="text-dark m-4 p-2 cal">
          ...
          <p className="lead mt-3 text-white article-con" style={{ whiteSpace: "pre-line" }}>
            {state.content}
          </p>
          {/* user comments */}
          <div>
            ...
            <div className="commentbox">
              {state.comments.length === 0 ? (
                <p className="cmt">No comments yet...</p>
              ) : (
                state.comments.map((commentObj, ind) => {
                  return (
                    <div key={ind} className="cmtbox p-3">
                      <p
                        className="fs-4"
                        style={{
                          color: "black",
                      ...
                      >
                        <FaRegCircleUser className="fs-2 me-2" />
                        {commentObj.username}
                      </p>
            ...
            <h1 className="comment-message">{comment}</h1>
            ...
            {currentUser.userType === "user" && (
              <form onSubmit={handleSubmit(writeComment)}>
                ...
                />
                <button type="submit" className="cmtbtn">
